...
SELECT 
	FIRST_NAME , PHONE_NUMBER ,EMAIL ,JOB_ID 
FROM EMPLOYEES 
...
SELECT 
	NAME , JIKWI , TEL 
FROM TBLINSA 
	WHERE CITY IN ('서울') AND JIKWI IN ('부장');
...
SELECT *FROM TBLINSA 
	WHERE BASICPAY || SUDANG >= 1500000 AND CITY IN ('서울') ;
...
SELECT * FROM TBLINSA
	WHERE BASICPAY * 12 >= 20000000 AND CITY IN ('서울') AND JIKWI IN ('과장', '부장'); 
